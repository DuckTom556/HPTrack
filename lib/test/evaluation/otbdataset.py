...
class OTBDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_info):
        ...
        #按序获得某个图像序列groundtruth路径
        anno_path = '{}/{}'.format(self.base_path, sequence_info['anno_path'])
        #获得ground truth数据并调整格式：[198,214,34,81]->[198. 214. 34. 81]<去掉,>
        # NOTE: OTB has some weird annos which panda cannot handle -OTB里有一些奇怪的标注格式，pandas 没法直接处理
        ground_truth_rect = load_text(str(anno_path), delimiter=(',', None), dtype=np.float64, backend='numpy')
            #delimiter=(',', None):表示文件可能用 , 号分隔，也可能没有分隔符（即空格/Tab）
        return Sequence(sequence_info['name'], frames, 'otb', ground_truth_rect[init_omit:,:],
                        object_class=sequence_info['object_class'])#构造一个 Sequence 对象，把 OTB 数据集的一段视频序列和它的标注信息封装起来。
