...
from lib.train.actors import HPTrackActor
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "hptrack":
        net = build_hptrack(cfg)
    else:
    ...
    if settings.script_name == "hptrack":
        ...
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT,
                       ...
                       'confidence': cfg.TRAIN.CONF_WEIGHT,
                       'focal': 1.,
                       }
        actor = HPTrackActor(net=net, objective=objective, loss_weight=loss_weight,
