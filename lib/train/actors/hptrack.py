...
class HPTrackActor(BaseActor):
    ...
    def compute_losses(self, pred_dict, gt_dict, return_status=True):
        ...
        gt_gaussian_maps_list = generate_heatmap(gt_dict['search_anno'],
        ...
        for i in range(len(pred_dict)):
            ...
            if 'confidence' in pred_dict[i] and pred_dict[i]['confidence'] is not None:
                pred_conf = pred_dict[i]['confidence'].view(-1)  # (BN,)
                conf_target = iou.detach()  # (BN,)
                confidence_loss = self.objective['mse'](pred_conf, conf_target)
            else:
                pred_conf=torch.tensor(0.0, device=l1_loss.device)
                confidence_loss = torch.tensor(0.0, device=l1_loss.device)
            # weighted sum
            loss = (self.loss_weight['giou'] * giou_loss
                    + self.loss_weight['l1'] * l1_loss
                    + self.loss_weight['focal'] * location_loss
                    + self.loss_weight['confidence'] * confidence_loss)
            total_loss += loss
            if return_status:
                # status for log
                mean_iou = iou.detach().mean()
                status = {f"{i}frame_IoU": mean_iou.item(),
                          f"{i}ConF": pred_conf.detach().mean().item(),
                          f"{i}Loss/confidence": confidence_loss.item(),
