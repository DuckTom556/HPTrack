...
class VisionMamba(nn.Module):
    ...
    def forward(self, x, len_search,len_hp,return_features=False, inference_params=None, if_random_cls_token_position=False,
                if_random_token_rank=False):
        xh=x[:,-len_search-len_hp:,:]
        zx=x[:,:-len_hp,:]
        '''hp=x[:,0:len_hp,:]
        xs=x[:,-len_search:,:]
        zx=x[:,len_hp:,:]
        xh=torch.cat([ xs, hp ], dim=1)'''
        output = self.forward_features(xh, inference_params, if_random_cls_token_position=if_random_cls_token_position,
                                  if_random_token_rank=if_random_token_rank)
        re_hp = output[:,-len_hp:,:]
        #re_x = torch.cat([czx,re_hp],dim=1)
        re_x = zx
        return re_x,re_hp
